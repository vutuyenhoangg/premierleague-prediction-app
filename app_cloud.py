...
def inject_hide_streamlit_cloud_ui():
    """
    Ẩn các chi tiết mặc định của Streamlit Cloud:
    - Nút Fork / GitHub góc trên phải
    - Toolbar / status / decoration mặc định
    - Các nút floating góc dưới phải của Streamlit Cloud

    Chỉ ẩn về mặt giao diện, không ảnh hưởng logic app.
    """
    st.markdown(
        """
        <style>
        /* Ẩn menu/header/tool mặc định của Streamlit */
        #MainMenu {
            display: none !important;
        ...
        /* Ẩn link/nút GitHub/Fork nếu nó nằm trong header */
        ...
        header div:has(a[href*="github.com"]),
        header div:has(button[title*="Fork"]),
        header div:has(button[aria-label*="Fork"]) {
            display: none !important;
            visibility: hidden !important;
            pointer-events: none !important;
        }
    ...
    components.html(
        ...
        <script>
        (function() {
            function hideStreamlitCloudChrome() {
                ...
                // Ẩn cụm Fork nếu CSS selector chưa bắt được.
                doc.querySelectorAll('a, button, div[role="button"]').forEach(function(el) {
                    const text = (
                        (el.innerText || '') + ' ' +
                ...
                // Ẩn các floating button góc dưới phải của Streamlit Cloud.
                doc.querySelectorAll('a, button, div[role="button"], div').forEach(function(el) {
                    ...
                    const isBottomRight =
                        ...
                        rect.width <= 180 &&
                        rect.height <= 120 &&
                        rect.right > win.innerWidth - 220 &&
                        rect.bottom > win.innerHeight - 140 &&
                        rect.top > win.innerHeight - 220;
...
inject_hide_streamlit_cloud_ui()
